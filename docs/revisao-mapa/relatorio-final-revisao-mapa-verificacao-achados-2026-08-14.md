# Relatório final de revisão do mapa de verificação de achados
...
> Este relatório consolida revisões produzidas com apoio de inteligência artificial. As propostas não substituem o juízo profissional da Equipe de Auditoria e somente devem ser incorporadas aos artefatos de produção depois de validação metodológica e jurídica, execução em cópia de teste e análise dos impactos sobre os resultados.

## 1. Objetivo e fontes examinadas

O objetivo é consolidar as alterações recomendadas para:
...
- os cinco arquivos `docs/revisao-mapa/revisao-critica-mapa-verificacao-achados*.md` existentes em 14/08/2026;
...
- `01-Planejamento/01-Estudos_Preliminares/Antecedentes/Acordao_44490_2024.PDF`;
- `01-Planejamento/01-Estudos_Preliminares/Antecedentes/Acórdão 1411 de 2014 Plenário.pdf`; e
...
O mapa é tecnicamente aproveitável, mas deve ser revisto antes de nova execução definitiva. A revisão final não recomenda eliminar nenhuma das questões Q1 a Q6, a questão transversal QT ou os achados A1 a A6. O problema principal não está na arquitetura temática, mas na correspondência entre o alcance dos títulos e o que os gatilhos efetivamente demonstram.

As alterações prioritárias são:

1. corrigir as lógicas de S2.2/S2.3, S4.1 e S5.3/S5.4, que hoje permitem dupla imputação ou conclusão mais ampla que os dados;
2. separar os quatro gatilhos atualmente repetidos em S3.5 e S6.3;
3. eliminar a cumulatividade excessiva em S2.1, S3.1, S4.3 a S4.5, S5.1, S5.2 e S6.1;
4. retirar do achado automático práticas de maturidade ou testes que não demonstram, isoladamente, a condição descrita;
5. eliminar ações sem função operacional e religar apenas as ações necessárias às regras recalibradas;
6. converter para **Determinação** apenas S2.2, S3.1 e S3.2, depois de restringidas a condições objetivas de inexistência ou não demonstração do Comitê de TIC e do processo/plano de TIC; e
7. manter as demais situações como **Recomendação**, sem prejuízo de determinação específica quando teste de processo concreto demonstrar descumprimento legal.

### 2.1 Respostas consolidadas aos quatro quesitos

| Quesito | Conclusão |
|---|---|
| 1. Ajustar ações de verificação? | **Sim.** Há 151 ações, das quais 41 não participam de `logica_achado`, `condicao_exibicao` ou `acoes_referencia`. A proposta final religa 9 delas e remove 32; a recalibração dispensa outras 37 ações hoje ativas. Restariam 82 ações, todas com função definida. |
...
| 3. Remover situação inconforme? | **Não é necessário suprimir tema inteiro.** Devem ser restringidas, renomeadas ou desdobradas várias situações. Elementos avançados deixam de gerar inconformidade automática e permanecem como indicadores de maturidade. |
| 4. Converter recomendação em determinação? | **Sim, em três situações:** S2.2, S3.1 e S3.2. As demais permanecem recomendações ou admitem determinação apenas após comprovação de ilegalidade em processo concreto. |

### 2.2 Oportunidades de melhoria priorizadas

| Prioridade | Oportunidade | Resultado esperado |
|---|---|---|
| Imediata | Corrigir S2.2/S2.3, S4.1, S3.5/S6.3 e S5.3/S5.4 | Eliminar dupla imputação, falsos positivos e uso do mesmo fato em situações distintas. |
| Alta | Recalibrar as regras multifatoriais e retirar práticas avançadas do achado automático | Aproximar a régua do mínimo essencial sem perder informação diagnóstica de maturidade. |
...
| Média | Reduzir e sanear o conjunto de ações | Passar de 151 para 82 ações, sem ações ou variáveis órfãs. |
...
A classificação deve observar a natureza da providência, e não apenas a severidade da situação:
...
O Acórdão TCE-RJ nº 44.490/2024-PLEN oferece a distinção mais aderente ao caso:

- nos itens II.1 e III.1, determinou a instituição de Comitê de TI quando inexistente;
- nos itens II.3 e III.3, determinou a instituição de processo estruturado destinado a elaborar, manter e revisar PDTI, com conteúdo mínimo e aprovação pela autoridade máxima;
- para Maricá, que já possuía controles, mas apresentava incompletudes, o item I.10 tratou o aperfeiçoamento do comitê, do PDTI, de ativos e de configuração como recomendações.

O Acórdão TCU nº 1.411/2014-Plenário reforça a relevância do planejamento de TIC, mas seus itens finais 9.1.5 e 9.1.6 estão no bloco iniciado por “recomendar”. Portanto, não deve ser citado como se o item 9.1.6 fosse, por si só, uma determinação. O suporte mais direto à classificação mandamental, no universo desta fiscalização, é o precedente do próprio TCE-RJ.

A Lei nº 14.133/2021 também exige cautela:

- o art. 11, parágrafo único, impõe à alta administração governança, processos e estruturas para as contratações;
- o art. 18 torna obrigatório o planejamento da fase preparatória e a compatibilidade com o PCA, **sempre que este for elaborado**, e com as leis orçamentárias;
- o art. 7º exige a designação de agentes para funções essenciais, mas não cria, de forma universal, a composição específica “integrante requisitante + integrante técnico de TIC” prevista em q2804[C]; e
- o art. 19, IV, dirige a obrigação de instituir modelos padronizados aos órgãos com competências regulamentares sobre licitações e contratos, o que não autoriza atribuí-la indistintamente a todos os jurisdicionados.

## 4. Questões de auditoria e achados

| Objeto | DE | PARA | Motivação/justificativa |
|---|---|---|---|
| Q1/A1 | Estrutura de TIC, sem alteração temática | **Manter** | A questão é respondível e os três controles são pertinentes. Ajustar apenas S1.2 e S1.3. |
...
| Q3/A3 | Processo formal, plano, conteúdo, integração e monitoramento | **Manter Q3/A3 e criar S3.3** para participação/priorização | Permite reservar a determinação ao núcleo objetivo de formalização do processo/PDTI e manter deficiências qualitativas como recomendação. Também utiliza a lacuna histórica do identificador S3.3 sem renumerar os demais itens. |
...
| Q6 | “planejamento, contratação, fiscalização e gestão” | “controles mínimos da fase preparatória das contratações de TIC, com planejamento, alinhamento e participação técnica proporcional ao risco” | Os gatilhos mais defensáveis concentram-se na fase preparatória. q2801[C], [D] e [E] alcançam seleção e gestão contratual e não devem ampliar automaticamente o achado A6. |
| A6 | “Fragilidades na governança técnica da fase preparatória...” | **Manter** | O título atual de A6 já está mais restrito e deve orientar a revisão de Q6, S6.1 e dos procedimentos. |
| QT | Levantamento longitudinal sem achado | **Manter sem alteração** | A natureza `levantamento` e `gera_achado: false` está coerente. |

Não se recomenda renumerar os identificadores estáveis apenas para eliminar lacunas. A criação de S3.3 resolve a lacuna material da Q3; os demais IDs devem ser preservados para não romper rastreabilidade histórica.

## 5. Situações inconformes: alterações consolidadas

As expressões abaixo usam os IDs atuais das ações. A redação final das ações deve ser atualizada para a nova situação a que passarem a pertencer.
...
| S4.1 | `total_TI == 0 \| total_SI == 0` | “Ausência de força de trabalho predominantemente dedicada à TIC em organização com área própria de TIC”; `AV26 & AV25`; AV26 deve excluir q0101 C e F | q0105 manda registrar quem atua nas duas áreas apenas na área predominante; `total_SI == 0` não prova ausência da função. O gate também evita duplicidade com S1.1 e falso positivo em modelo centralizado externo. |
...
## 6. Alterações nas ações e fórmulas do mapa

### 6.1 Fórmulas `logica_achado`
...
| PA02 | `(((AV07\|AV85)\|(AV08\|AV86)\|(AV09\|AV87)\|(AV10\|AV88)) \| (AV11\|AV89) \| ((AV12\|AV90)&(AV13\|AV91)))` | `((AV07\|AV85) \| (AV08\|AV86) \| ((AV09\|AV87)&(AV10\|AV88)) \| (AV11\|AV89) \| (AV12&~AV89&(AV13\|AV91)))` | Separa monitoramento e impede que evidência insuficiente da instituição seja usada como prova de existência do comitê. |
| PA03 | Quatro itens de q2101 e quatro itens repetidos em S3.5/S6.3 | `((AV17\|AV95) \| (AV14\|AV92) \| (AV15\|AV93) \| (AV18\|AV96) \| (AV19\|AV97) \| (AV20\|AV98) \| (AV24\|AV101))` | Separa formalização de qualidades do processo e elimina a duplicidade com S6.3. |
...
O levantamento programático confirmou 110 ações atualmente utilizadas e 41 órfãs. A solução não é remover indiscriminadamente todas as órfãs: nove são necessárias às regras recalibradas.
...
| Religar | AV35, AV37, AV109, AV111 | Órfãs | Núcleo de S4.4 | Testam competências/habilidades de perfis, sem exigir publicação ou forma de seleção. |
...
### 6.4 Motivos do relatório

A aba `Motivos do Relatório` deve ser ajustada em conjunto com as fórmulas. Não basta alterar `logica_achado`, pois motivos incompatíveis produziriam narrativas contraditórias.
...
| `Ações de Verificação`, S6.2 | Encaminhamento inclui “segurança da informação”; matriz não | Adotar a mesma redação nos dois artefatos | Eliminar divergência textual entre matriz e mapa. |

## 7. Alterações correspondentes na matriz de planejamento
...
| Bloco/campo | DE | PARA | Motivação/justificativa |
|---|---|---|---|
| Q1 — S1.2 | Descrição ampla de todas as funções de TIC | Descrição limitada a ausência geral de atribuições ou ausência de governança, planejamento e gestão | Alinhar o texto a q0103[G]/[D]. Ajustar E3/E4 e o encaminhamento na mesma direção. |
| Q1 — S1.3 | Regra q0102 B/C/D/E | Regra q0102 D/E | Harmonizar matriz e mapa e afastar posições estratégicas ou administrativas não conclusivas. |
| Q2 — riscos, procedimentos e evidências | R2.1/P1/P2/E1/E2 misturam direção e monitoramento | Separar o risco de falta de direção do risco de ausência de monitoramento; criar referências para S2.4 | Sustentar o desdobramento sem perda de rastreabilidade. |
| Q2 — S2.1/S2.4 | Uma situação com quatro subitens em OR | S2.1 com q1001[C]/[H]; S2.4 com q1002[A] e [C] em AND | Mesma lógica do mapa. |
| Q2 — S2.2 | “não instituído”; Recomendação | “não instituído ou não demonstrado formalmente”; **Determinação** | A matriz deve refletir tanto resposta negativa quanto avaliação documental insuficiente. |
| Q2 — S2.3 | Apenas `(E == Sim) & (F != Sim)` | Exigir também comprovação suficiente da instituição e admitir avaliação não conforme da atuação | Evitar que evidência insuficiente da existência habilite teste de funcionamento. |
| Q3 — R/IR/P/E/S3.1 | Processo formal e atributos qualitativos unidos | S3.1 restrita a q2101[D]; criar R3.3, IR6, P6, E6 e S3.3 para participação/priorização; q2101[C] fica fora do achado automático | Restabelecer rastreabilidade e proporcionalidade. |
| Q3 — S3.1 | Recomendação genérica | **Determinação** para estabelecer processo estruturado destinado a elaborar, manter e revisar PDTI/PEDTIC ou equivalente | Precedente direto do TCE-RJ. |
| Q3 — S3.2 | Ausência de aprovação; Recomendação | Inexistência/não demonstração/falta de aprovação; **Determinação** para elaborar/submeter/aprovar formalmente PDTI/PEDTIC ou equivalente | Precedente direto do TCE-RJ e redação aderente ao dado. |
| Q3 — IR8/P8/E8/S3.5 | Incluem q2102, q2802 e q2804 | Restringir a q2102[C]/q2102evi | Os dados de PCA e contratação concreta ficam exclusivamente em Q6. |
| Q4/A4 — títulos | Capacidade institucional insuficiente | Planejamento e gestão da força de trabalho e das competências | Evitar conclusão material não mensurada. |
| Q4 — S4.1 | TI ou SI igual a zero | q0101 fora de C/F e total_TI igual a zero; retirar `total_SI` | Respeitar a instrução de preenchimento de q0105 e modelos compartilhados. |
| Q4 — S4.2 | Severidade alta | Severidade média | Fragilidade de planejamento, não ausência material comprovada. |
| Q4 — S4.3 | B ou D ausente; descrição inclui cargos efetivos | B e D simultaneamente ausentes; descrição limitada a função formal | Evitar ingerência sobre criação de cargos e admitir SI integrada. |
| Q4 — IR4/IR5/IR6, P4/P5, E4/E5 e S4.4 | Publicação de perfis e seleção de gestores | Competências/habilidades dos perfis em q2701[C]/q2702[C]; retirar q2704 do achado | Publicação e forma de seleção permanecem no questionário como maturidade. |
| Q4 — IR7/IR8, P6, E6 e S4.5 | Liderança, técnicas e plano de capacitação | Lacunas técnicas q2705[C]/[D] | Não confundir plano de treinamento com todas as formas de tratamento. |
| Q4 — IR9/P7/E7/S4.6 | Modelos B/C ou predomínio de terceiros | Modelo B e ausência de pessoal interno | O teste final não deve afirmar incapacidade com base apenas em predominância numérica. |
| Q5 — subquestões e IR/P/E | Catálogo/ANS/configuração tratados como requisitos uniformes | Distinguir catálogo disponível, metas ou ANS alternativos, inventário e formalização proporcional da configuração | Ajustar o desenho à heterogeneidade do universo. |
| Q5 — S5.1/S5.2 | A/B/C e D/E em OR | S5.1 por q2201[C]; S5.2 por ausência simultânea de q2201[A] e q2201[D] | Eliminar sobreposição catálogo–ANS e aceitar solução equivalente. |
| Q5 — S5.3/S5.4 | q2203[A] nas duas situações | S5.3 com q2501[A]/q2504[A]/[B]; S5.4 com q2203[C] | Eliminar dupla contagem e CMDB como mínimo universal. |
| Q5 — S5.5 | Serviços e segurança juntos | S5.5 para incidentes de serviços e nova S5.6 para incidentes de segurança | Produzir encaminhamentos específicos e rastreáveis. |
| Q6 — questão/subquestões/riscos | Planejamento, seleção, fiscalização e gestão | Controles mínimos da fase preparatória, alinhamento e participação técnica proporcional | Coerência com A6 e com os itens efetivamente mantidos. |
| Q6 — IR1/IR2/IR3/P1/P2/E1/E2/S6.1 | q2801[A]/[C]/[D]/[E]/[G] | q2801[A] e respectiva evidência | Restringir S6.1 à fase preparatória. |
| Q6 — S6.2 | Obrigatoriedade universal; severidade alta | Regra geral não demonstrada; severidade média; redação proporcional e alinhada à do mapa | O teste não substitui exame de processos concretos. |
| Q6 — IR6/P4/E5/S6.3 | Inclui q2102[C] | Excluir q2102[C]; manter q2802[C]/[D], q2804[B] e evidências correspondentes | Separar planejamento de TIC de execução do PCA/contratações. |
| Q6 — S6.4 | Severidade alta e equipe específica universal | Severidade média e cláusula de aplicabilidade conforme complexidade/risco | Art. 7º da Lei nº 14.133/2021 não impõe a composição literal de q2804[C] a toda contratação. |
| Todas as situações alteradas | `tipo_encaminhamento` e textos atuais | Replicar exatamente o tipo e o texto aprovados no mapa | Evitar divergência entre fonte de planejamento e instrumento de execução. |
...
As seis linhas de ação acima devem receber a mesma classificação na planilha. Não se deve manter ações de uma mesma situação com tipos distintos.
...
| S2.1, S2.3 e S2.4 | Tratam de conteúdo, funcionamento e monitoramento de mecanismos existentes. O Acórdão TCE-RJ nº 44.490/2024 tratou aperfeiçoamentos de controles existentes como recomendações. |
...
| Proposta encontrada | Conclusão final | Justificativa |
|---|---|---|
| Usar incidência de 100% como prova de erro | **Não acolher como prova; usar como sinal de revisão** | Universalidade pode refletir baixa maturidade real. A decisão deve se apoiar na validade do gatilho, não apenas na frequência. |
...
| Converter S1.1/S1.2 em determinação com base apenas em eficiência e art. 11 da Lei nº 14.133/2021 | **Não acolher** | O art. 11 trata governança das contratações e não autoriza o Tribunal a desenhar toda a estrutura administrativa de TIC. |
| Converter S2.3 em determinação | **Não acolher automaticamente** | O precedente local determinou instituir comitê inexistente, mas recomendou aperfeiçoar funcionamento/monitoramento de comitê existente. |
| Converter S4.1/S4.6 em determinação | **Não acolher automaticamente** | Quantidade/predominância não prova incapacidade nem ausência de fiscalização contratual. |
| Converter S5.3/S5.5 em determinação com base genérica em LGPD/Lei nº 4.320/1964 | **Não acolher automaticamente** | O escopo dos gatilhos é mais amplo que o dever legal específico. Determinação depende de condição concreta e norma aplicável. |
| Converter S6.4 em determinação com base no art. 7º da Lei nº 14.133/2021 | **Não acolher automaticamente** | O artigo exige agentes qualificados e segregação, mas não a equipe específica de q2804[C] para toda contratação. |
| Manter S3.5 e S6.3 idênticas apenas com nota metodológica | **Não acolher** | A separação dos itens é simples, preserva as duas subquestões e elimina a aparência de dupla imputação. |
...
| Renumerar todos os IDs para eliminar lacunas | **Não acolher no ciclo atual** | A estabilidade dos IDs é mais valiosa para rastreabilidade. Criar S3.3 e documentar eventuais lacunas restantes. |

## 10. Ordem de implementação e validação

1. Editar primeiro `matriz_planejamento.md`, mantendo-a como fonte de verdade.
2. Aplicar, em cópia do XLSX sob `/tmp/tcerj-igovti-2026`, as mudanças nas fontes, variáveis, ações, procedimentos e motivos.
...
5. Atualizar o catálogo YAML de prompts, sem editar os Markdown gerados diretamente; manter apenas avaliações ainda consumidas pelo mapa e regenerar os prompts.
...
10. Ajustar os templates para distinguir determinação de recomendação, inclusive quanto a prazo, plano de ação e consequência do descumprimento.
...
- [ ] As 69 ações indicadas foram removidas e as 9 órfãs úteis foram religadas.
